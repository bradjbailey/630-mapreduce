...
import org.apache.hadoop.yarn.util.resource.Resources;
 ...
 public class LongestJobFirstScheduler extends
     AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode> implements
     Configurable {
 
   private static final Log LOG = LogFactory.getLog(LongestJobFirstScheduler.class);
   ...
   public LongestJobFirstScheduler() {
     super(LongestJobFirstScheduler.class.getName());
   }
     ...
     LOG.debug("assignContainers:" +
        ...
        Collections.sort(entryList, new Comparator<Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>>>() {
            ...
            public int compare(Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> entry1, Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> entry2) {
                ...
                long a1MemConsump = a1.getCurrentAppAttempt().getCurrentConsumption().getMemory();
                long a1MemRes = a1.getCurrentAppAttempt().getCurrentReservation().getMemory();
                long a1MemUsage = a1MemConsump + a1MemRes;
    
                // get resource usage of a2
                long a2MemConsump = a2.getCurrentAppAttempt().getCurrentConsumption().getMemory();
                long a2MemRes = a2.getCurrentAppAttempt().getCurrentReservation().getMemory();
                long a2MemUsage = a2MemConsump + a2MemRes;
    
                // compare
                if (a1MemUsage > a2MemUsage) {
                    return -1;
                } else if (a1MemUsage < a2MemUsage) {
                    return 1;
                } else {
                    return 0;
                }
   ...
   @Lock(LongestJobFirstScheduler.class)
   @Override
