...
import org.apache.hadoop.yarn.util.resource.Resources;
 ...
 public class HighestVCoresJobFirstScheduler extends
     AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode> implements
     Configurable {
 
   private static final Log LOG = LogFactory.getLog(HighestVCoresJobFirstScheduler.class);
   ...
   public HighestVCoresJobFirstScheduler() {
     super(HighestVCoresJobFirstScheduler.class.getName());
   }
   ...
   private void assignContainers(FiCaSchedulerNode node) {
     LOG.debug("assignContainers:" +
        ...
        // Get the entrySet() of the ConcurrentMap
        Set<Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>>> entrySet = applications.entrySet();
        // Create a List to store the entries
        List<Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>>> entryList = 
            new ArrayList<Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>>>(entrySet);
        // Sort the entryList based on memory usage of the FiCaSchedulerApp
        // DESCENDING
        Collections.sort(entryList, new Comparator<Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>>>() {
            @Override
            public int compare(Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> entry1, Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> entry2) {
                SchedulerApplication<FiCaSchedulerApp> a1 = entry1.getValue();
                SchedulerApplication<FiCaSchedulerApp> a2 = entry2.getValue();
    
                // get resource usage of a1
                long a1CPUConsump = a1.getCurrentAppAttempt().getCurrentConsumption().getVirtualCores();
                long a1CPURes = a1.getCurrentAppAttempt().getCurrentReservation().getVirtualCores();
                long a1CPUUsage = a1CPUConsump + a1CPURes;
    
                // get resource usage of a2
                long a2CPUConsump = a2.getCurrentAppAttempt().getCurrentConsumption().getVirtualCores();
                long a2CPURes = a2.getCurrentAppAttempt().getCurrentReservation().getVirtualCores();
                long a2CPUUsage = a2CPUConsump + a2CPURes;
    
                // compare
                if (a1CPUUsage > a2CPUUsage) {
                    return -1;
                } else if (a1CPUUsage < a2CPUUsage) {
                    return 1;
                } else {
                    return 0;
                }
        ...
        // entry list is sorted by descending memory usage
       for (Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> e : entryList) {
   ...
   @Lock(HighestVCoresJobFirstScheduler.class)
   @Override
